...
| 结果 | 套件 | 事实 |
|---|---|---|
| ✅ PASS | 服务端单测 | tests=1887　pass=1884　fail=0　skipped=3 |
...
## 跳过原因
- **红队实战评测（ground-truth 真值对照 + sqlmap 同题）**：红队评测靶场未常驻（127.0.0.1:8231）——先执行 npm run lab:redteam
- **fileRead 真闭环**：secure_file_priv 未放行（MySQL 8 默认 NULL）→ 本套件未执行任何断言
- **fileWrite 真闭环（文件系统侧断言）**：secure_file_priv 未放行（MySQL 8 默认 NULL）→ 本套件未执行任何断言
