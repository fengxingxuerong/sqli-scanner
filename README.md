...
[![Tests](https://img.shields.io/badge/tests-2512%20passing-brightgreen)](#测试)
...
- 服务端测试: 2197 用例（2194 pass / 0 fail / 3 skip，并发口径 2026-09-22 复测；3 skip 为环境依赖显式跳过。覆盖率 lines 90.04 / branch 76.88 / func 79.06，阈值 85/69/72）
...
**扫描覆盖 22/22**（2026-09-20 补齐原先从未被扫描的 `D1-postform` / `E1b-admin-query`，
见 TODO §W）—— 由 `npm run targets:check` 持续守护该差集，不再静默；
补齐后这两点在 r1 / r2 两轮**均能检出**。

> 这个 22/22 是**修掉一处标定错误**之后的结果：`E1b` 原先被标为二阶注入并用 `--second-order` 扫，
> 必然 MISS。查证（靶场代码 + 手工实测）表明它其实是「**需 admin 会话的普通 query 注入**」——
> 靶场把写入字段放在 SELECT 列表，`WHERE` 用的是 query 的 `status`，写入不影响查询结构。
> 改按 query 注入接入后即命中 `union+error`（dbms=MySQL）。
> 真值标定因此重建（**漏洞点 15/15、安全点 7/7 仍全部成立**，靶点 SQL 形态一字未改）。
...
> ⚠️ **本节在 2026-09-19 被整体推翻过一次。** 此前对外引用的「tamper off 2 → tamper on 8」
> 「自动选链 11」出自保真度只有 **60.7%** 的自实现 SecRule 执行器（两处结构性缺陷：`(?i)`
> 内联大小写标记被吃掉、链节点的 `TX/MATCHED_VARS` 未实现）。用 CRS 官方回归集把执行器修准后
> （保真度 99.3%），**同一批探针在同一份规则上的结论完全变了** —— 「绕过有效」是宽松执行器
> 白给的假收益。旧归因（含 942361 的规则级分析）保留在 TODO §F 作历史记录，**不再作为对外口径**。
...
| 根因 | 说明 |
|---|---|
| CRS pattern 用 PCRE 的 `(?i:…)` 组级大小写开关 | 本仓跑在 **Node 22.22.2 / V8 12.4**，该语法（ES2025 regex modifiers，需 V8 13 / Node 23+）直接抛 `Invalid group` |
| 执行器 catch 后返回 null | 命中失败的 5 条规则（942160 / 942220 / 942250 / 942361 / 942450）**恒不命中** → 官方回归集漏 23 条 |

修复 = `toJsRegex()` 统一把 `(?i)` 删掉、`(?i:…)` 降级为 `(?:…)`（本执行器恒以 `flags:'i'` 编译，二者等价），
...
需要改起始形状的算子」这一结论在新口径下不再需要 —— PL3 下所有探针全拦（0/0），PL1 下无需绕过（8/8）。

**自动路径（不显式配 tamper）**（`npm run waf-auto`）：
...
**本轮 0 FAIL。** 上一轮的两个 FAIL 均已定位并修复（都不是被测代码的随机波动，是可复现的确定性问题）：

1. `CRS 执行器保真度` —— 96% / 23 条未点名分歧 → **99.3% / 0**。病根是 CRS pattern 里的 PCRE
   组级大小写开关 `(?i:…)` 在 Node 22（V8 12.4）上编译失败，5 条规则恒不命中。详见上文 WAF 节。
2. `tests/engine.e2e.test.js` 环境耦合假红 —— 端口写死 4567 并复用该端口上已存在的带 token 实例。

> 同轮跑出的另一条 FAIL **已修**：`tests/engine.e2e.test.js` 旧实现把引擎端口写死 4567 **并复用**该端口上
> 已存在的进程 —— 本机 4567 跑着一个带 `SCAN_API_TOKEN` 的实例，受保护端点返 401，而用例断言的是 2001，
> 与被测代码无关（另起无 token 实例请求同一路径返回 `{"code":2001}`）。现改为「取空闲端口 + 自己 spawn
> 带一次性 token 的实例 + 请求显式带 token」，并补两条鉴权契约用例；修后单文件连跑 5 次 7/7、
> 门禁内该套件 1892 pass / 0 fail。这类「测试复用外部进程 → 结果由环境决定」的假红形态值得单独警惕。

> 更早那次「8 PASS / 3 SKIP」是在 MySQL 未起、红队靶场未常驻的环境下跑的，与本次不是同一环境的两个样本。
> 再早的「11 PASS / 0 SKIP」则是假绿：2026-09-17 那次是在 `secure_file_priv=''` 的放行实例
...
> 套件说成验过了**。（本轮 2026-09-20 00:18 已在同一环境下跑到 12 PASS / 0 SKIP —— 靠的是补齐环境
> 与修掉真实缺陷，不是把 SKIP 改回 PASS。）
