// ============================================================================
// scan/detect.js —— 检测调度 + WAF 自适应重跑（原 scanRunner.runScanLoop 阶段 2 + 3.4）
//
// 从 runScanLoop 封边（2026-09-12 状态收拢）。**纯搬移，行为不变**：
//   · 输入 = run（cfg/ctxBase/guard/pointsToScan/rawClient/s/session/target/validity/validityEnabled）
//   · 输出 = 检测产物对象（foundByPoint/blockPolicy/blockAdaptiveInfo/dbms/extracted/
//     fullyTestedPoints/oobUnavailable/stackedSelected/wafAgg + scheduler）
// 37 个散变量中仅这 9+1 个跨段，其余中间变量全部留在本函数内部 —— 封边后内部重构自由。
// scheduler 通过 run.schedulerRef 回写：db-guard 的 onTrip 闭包延迟读取它（时序安全）。
// ============================================================================
 ...
 * @returns {Promise<object>} 检测产物（挂回 run 供下游阶段消费）
 */
export async function detectPhase(run) {
  const { cfg, ctxBase, guard, pointsToScan, rawClient, s, scanId, session, sm, target, validity, validityEnabled } = run;
    ...
    run.schedulerRef = scheduler; // [熔断] 绑定守卫的降并发目标
