...
> 📌 **后续**：本文件的思路在 2026-09-20~21 又续了五刀（httpClient / Extractor /
> ScanManager / ReportGenerator / scanRoutes，净减 665 行），并沉淀出一套
> 「耦合度实测选簇 + 薄委托保路径 + 缺陷注入验证」的流程。
> 见 **[大文件拆分-五刀复盘-2026-09-21.md](./大文件拆分-五刀复盘-2026-09-21.md)**。
> 下文 §实施进度 中 detect.js 的「建议不做」结论**已被推翻**，详见该节订正。
...
动手前先做了静态耦合分析（统计待抽段落在主函数作用域引用的变量数）：

| 段落 | 行数 | 引用主函数变量 | 结论 |
|---|---|---|---|
| detect（阶段2 + 3.4 整体） | 550 | **37** | 耦合过高 |
| wafAdaptive（仅 3.4） | 288 | 25 | 高 |
| scheduler 检测循环（仅阶段2） | 262 | 25 | 高 |
| **discover（阶段1）** | 173 | **17** | ✅ 已做 |
| context（阶段0） | 150 | 14 | 中低，收益低 |

**判断依据**：那 37 个变量里有 15+ 个是 detect 段**自己产出**的
（`blockAdaptive` / `blockHits` / `errorOnlyPoints` / `fiveXx` / `highConfWaf` /
`filterAdaptive` / `autoRetryEnabled` / `blockAdaptiveInfo` / `sharedBaseline` /
`validitySnap` / `wafVendorsPre` / `stackedSelected` / `fullyTestedPoints` …），
下游阶段 3 与 finalize 都要用。

纯搬移的后果：主函数行数变少，但**总复杂度不变**——只是把 37 个散变量从闭包换成
`run` 对象的 37 个字段，再叠一层跨文件跳转。收益（主函数可读）小于风险（检测主循环回归）。

**真正的问题不是"函数太长"，而是"阶段间靠 37 个散变量通信"**。要根治得先把这些状态
按语义收拢成 3–4 个显式对象（如 `run.waf` / `run.progress`），但那属于**改数据流组织**，
不是纯搬移 → 行为等价性验证不再适用，风险显著高一档，应单独立项、单独设计验证。

因此本轮到此收手：已完成的是"低风险高收益"的部分（-405 行、5 个模块、全程行为等价验证通过）。
